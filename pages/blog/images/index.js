...
export default ({imagePosts}) =>
	{
		if(!Object.keys(imagePosts).length) return (<div>Missing Props</div>);
...
export async function getStaticProps() {
	const apiPath = "/blog/api/getImagePosts";

	try {
		const	imageRes 	= await fetch(process.env.global.apiURL + apiPath),
